...
async def process_pipeline(self, index_id: str, pipeline: dict, payload: dict):
    ...
    #     {"pipeline_tasks": pipeline, "task_id": task_id, "status": "PROCESSING"}
    # )
    ...
    await process_orchestrator(index_id, "task_id", pipeline, payload)
